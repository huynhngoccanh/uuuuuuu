class Admins::CjAdvertiserController < Admins::ApplicationController
...
before_action :set_advertiser, only: [:show, :edit, :update, :destroy]
  ...
  def update
    respond_to do |format|
      if @advertiser.update(advertiser_params)
        format.html { redirect_to edit_admins_cj_advertiser_path(@advertiser), notice: 'Advertiser was successfully updated.' }
  ...
  private
    ...
    def set_advertiser
      @advertiser = CjAdvertiser.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def advertiser_params
      params[:cj_advertiser].permit!
    end
end