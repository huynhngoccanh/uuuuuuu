class Admins::LinkshareAdvertiserController < Admins::ApplicationController
...
before_action :set_advertiser, only: [:show, :edit, :update, :destroy]
  ...
  def update
    respond_to do |format|
      if @advertiser.update(advertiser_params)
        format.html { redirect_to edit_admins_linkshare_advertiser_path(@advertiser), notice: 'Advertiser was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.html { redirect_to admins_linkshare_advertiser_index_path }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def set_advertiser
      @advertiser = LinkshareAdvertiser.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def advertiser_params
      params[:linkshare_advertiser].permit!
    end
end