class LoyaltyCrawler::Jetblue
  ...
  def get_history
    ...
    begin
      ...
      begin
        s.find('.points')
        @loyalty_programs_user.update_attributes(account_number:"N/A", points: s.find('.points').text ,status: "success")
      rescue
        s.find('#true_blue_form')
        @loyalty_programs_user.update_attributes(status: "failed", exception: s.find('#true_blue_form').text)
      end 
    rescue Exception => e
